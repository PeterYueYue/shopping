...
<div id="goods-filter-box">
    <ul id="sort-result">
        <li class="s-type" v-if="filterType=='menu'" @click="isShow=!isShow">
            <span class="cur">{{filterState.proClass}}</span>
            <div class="arrBox proName">
                <i :class="isShow?'uptrg':'downtrg'"></i>
            </div>
        </li>
        <li class="s-v s-up" @click="filterProduct(filterState.categoryId,filterState.proClass,filterState.sortType==2?3:2)">
            <span>抵扣数</span>
            <div class="arrBox">
                <i class="uptrg" :class="filterState.sortType ==2?'bgT':''"></i>
                <i class="downtrg" :class="filterState.sortType==3?'bgB':''"></i>
            </div>
        </li>
        <li class="s-sale" @click="filterProduct(filterState.categoryId,filterState.proClass,filterState.sortType==4?5:4)">
            <span>销量</span>
            <div class="arrBox">
                <i class="uptrg" :class="filterState.sortType==4?'bgT':''"></i>
                <i class="downtrg" :class="filterState.sortType==5?'bgB':''"></i>
            </div>
        </li>
        <li class="s-p" @click="filterProduct(filterState.categoryId,filterState.proClass,filterState.sortType==6?7:6)">
            <span>价格</span>
            <div class="arrBox">
                <i class="uptrg" :class="filterState.sortType==6?'bgT':''"></i>
                <i class="downtrg" :class="filterState.sortType==7?'bgB':''"></i>
            </div>
        </li>
        <li class="s-p" v-if="filterType=='search'" @click="$emit('showSearchFilte')">
            <span>筛选</span>
        </li>
    </ul>
