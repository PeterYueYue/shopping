...
<div id="products-list-box">
    <ul v-infinite-scroll="loadMore" infinite-scroll-disabled="busy" infinite-scroll-distance="10">
        <li v-for="data in proData">
            <router-link :to="'/goodsDetail/pro/'+data.prodId">
                <img :src="data.prodImg|imgFilter(1)">
                <p class="proName" v-html="data.prodName"></p>
                <p class="subName"><span>销量{{data.prodSoldCount}}</span></p>
                <p class="proPrice">
                    ¥<b>{{data.prodSpecOrigPrice}}</b><del>¥{{data.prodSpecOrigPrice}}</del><span>可抵用{{data.specVouchers}}优券</span>
                </p>
            </router-link>
        </li>
    </ul>
