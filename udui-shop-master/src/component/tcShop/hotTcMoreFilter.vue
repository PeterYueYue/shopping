        ...
        <transition name="app-file">
            <div class="sec-screen" v-show="isShow.show">
                <div class="sec-type clearfix" v-show="isShow.num==1">
                    <div class="pare">
                        <ul class="">
                            <li @click="showShopClass(0)"><p @click="filterShop('categoryId',0)">全部分类</p></li>
                            <li :class="data.id == isShow.shopClassShow ? 'cur':''" @click="showShopClass(data.id)"
                                v-for="data in shopClass">{{data.name}}
                            </li>
                        </ul>
                    </div>
                    <div class="child">
                        <div class="scroller">
                            <ul v-show="data.id == isShow.shopClassShow" v-for="data in shopClass">
                                <li>全部</li>
                                <li v-for="d in data.childList" @click="filterShop('categoryId',d.id)">{{d.name}}</li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="sec-trade clearfix" v-show="isShow.num==2">
                    <div class="pare">
                        <ul>
                            ...
                            <li :class="data.id == isShow.areaShow ? 'cur':'' " @click="getBusiness(data.id)"
                                v-for="data in tcArea">{{data.name}}
                            </li>
                        </ul>
                    </div>
                    <div class="child">
                        <div class="scroller">
                            <ul>
                                <li v-for="data in areaBusinessList" @click="filterShop('tradeId',data.id)">
                                    {{data.name}}
                                </li>
                            </ul>
                        </div>
                ...
                <div class="sec-sort clearfix" v-show="isShow.num==3">
                    ...
                    <div class="child" id="sortChild">
                        <div class="scroller">
                            <ul>
                                <li @click="filterShop('intelligentSort',1)">
                                    按距离排序
                                </li>
                                <li @click="filterShop('distanceSort',1)">
                                    按销量排序
                                </li>
                            </ul>
                        </div>
