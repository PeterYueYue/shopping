...
<template>
    ...
    <div id="goods-filter-box">
        <ul id="sort-result">
            ...
            <li class="s-v s-up" @click="doSort('xiaoliang')">
                <span>销量</span>
                <div class="arrBox">
                    <i class="uptrg" :class=" glFilter.soldSort == 1?'bgT':''"></i>
                    <i class="downtrg" :class="glFilter.soldSort == 2?'bgB':''"></i>
                </div>
            </li>
            <li class="s-sale"  @click="doSort('jiage')">
                <span>价格</span>
                <div class="arrBox">
                    <i class="uptrg" :class="glFilter.priceSort==1?'bgT':''"></i>
                    <i class="downtrg" :class="glFilter.priceSort==2?'bgB':''"></i>
                </div>
            </li>
            <li class="s-p" @click="doSort('zhekoujie')">
                <span>抵扣数</span>
                <div class="arrBox">
                    <i class="uptrg" :class="glFilter.deduceSort==1?'bgT':''"></i>
                    <i class="downtrg" :class="glFilter.deduceSort==2?'bgB':''"></i>
                </div>
            </li>
        </ul>
            ...
            <div class="sec-screen" v-show="isShow">
                <div class="sec-type">
                    <ul>
                        ...
                        <li :class="parentId==data.id?'cur':''" @click="filShow(data.id)"  v-for="data in navData">
                            {{data.name}}
                        </li>
                    </ul>
                </div>
                <div class="child" v-for="data in navData" v-show="data.id == parentId">
...
//                productName: '',
        ...
        methods: {
            ...
            doSort(val){
                if(val == 'xiaoliang'){
                    this.glFilter.soldSort =  this.glFilter.soldSort == 1 ? 2 : 1;
                    this.glFilter.priceSort = this.glFilter.deduceSort = undefined;
                }else if(val == 'jiage'){
                    this.glFilter.priceSort =  this.glFilter.priceSort == 1 ? 2 : 1;
                    this.glFilter.soldSort = this.glFilter.deduceSort = undefined;
                }else if(val == 'zhekoujie'){
                    this.glFilter.deduceSort =  this.glFilter.deduceSort == 1 ? 2 : 1;
                    this.glFilter.soldSort = this.glFilter.priceSort = undefined;
                }
                this.$emit('doSort',JSON.parse(JSON.stringify(this.glFilter)));
            },
            filterProduct(id, filterName, SkuSortEnum) {
                this.isShow = false;
                this.glFilter.categoryId = id;
                this.glFilter.proClass = filterName;
                console.log( JSON.parse(JSON.stringify(this.glFilter)));
                this.$emit('filterPro',JSON.parse(JSON.stringify(this.glFilter)));
            }
        },
        created(){

        },
        props: ['navData','glFilter']
    }
