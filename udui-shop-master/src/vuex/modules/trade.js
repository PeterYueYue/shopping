...
const mutations = {
    ...
    [types.Set_ShopCart](state,amount){
        ...
        if(amount!=null){
            ...
            for (var i = 0; i < state.shopCart.cartBySellerList.length; i++) {
                if(state.shopCart.cartBySellerList[i].sellerId == -1){  //如果购物车中有京东商品
                    let jdProTotalPrice = 0;
                    for (var j = 0; j < state.shopCart.cartBySellerList[i].shoppingCartDtoList.length; j++) {
                        let productCount = state.shopCart.cartBySellerList[i].shoppingCartDtoList[j].productCount //数量
                        let origPrice    = state.shopCart.cartBySellerList[i].shoppingCartDtoList[j].origPrice  //单价
                        let vouchers     = state.shopCart.cartBySellerList[i].shoppingCartDtoList[j].vouchers //单个商品可抵用优券数量
                        jdProTotalPrice += (origPrice - vouchers)*productCount;
                    }
                    if(jdProTotalPrice<99){
                        state.shopCart.cartBySellerList[i].qcdBoxActive = true;
                        state.shopCart.cartBySellerList[i].qcdBoxPrice = (99-jdProTotalPrice).toFixed(2);
                    }else{
                        state.shopCart.cartBySellerList[i].qcdBoxActive = false;
                    }
                }
            }
    ...
    [types.Set_order_pros](state,amount){
        ...
        for (var i = 0; i < state.order.length; i++) {
            if(state.order[i].sellerId == -1){  //如果购物车中有京东商品
                let jdProTotalPrice = 0;
                for (var j = 0; j < state.order[i].shoppingCartDtoList.length; j++) {
                    let productCount = state.order[i].shoppingCartDtoList[j].productCount //数量
                    let origPrice    = state.order[i].shoppingCartDtoList[j].origPrice  //单价
                    let vouchers     = state.order[i].shoppingCartDtoList[j].vouchers //单个商品可抵用优券数量
                    jdProTotalPrice += (origPrice - vouchers)*productCount;
                }
                if(jdProTotalPrice<99){
                    state.order[i].qcdBoxActive = true;
                    state.order[i].qcdBoxPrice = (99-jdProTotalPrice).toFixed(2);
                }else{
                    state.order[i].qcdBoxActive = false;
                }
            }
        }
    },
    ...
    [types.Set_order_payinfo](state,amount){
        console.log(amount);
        state.payInfo.payCash = amount.payInfo.totalPay;
        state.payInfo.payVouchers = amount.payInfo.totalVouchers;
        if(amount.payInfo.totalPay<amount.uCash){
            state.payInfo.payWay = 4;
        }else{
            if(amount.navg=='wx'){
                state.payInfo.payWay = 2;
            }
            if(amount.navg == 'ali'){
                state.payInfo.payWay = 1;
            }
        }
        console.log(state.payInfo.payWay);
    },
    [types.Set_order_payway](state,amount){
        state.payInfo.payWay = amount;
    },
    [types.set_cart_Spec](state,amount){
        for (var i = 0; i < state.shopCart.cartBySellerList.length; i++) {
            for (var j = 0; j < state.shopCart.cartBySellerList[i].shoppingCartDtoList.length; j++) {
                if (state.shopCart.cartBySellerList[i].shoppingCartDtoList[j].id == amount.cartId) {
                    state.shopCart.cartBySellerList[i].shoppingCartDtoList[j].productSpecId = amount.productSpecId;
                    state.shopCart.cartBySellerList[i].shoppingCartDtoList[j].productSpecName = amount.name;
                    state.shopCart.cartBySellerList[i].shoppingCartDtoList[j].productImg = amount.productImg;
                }
            }
        }
    }
